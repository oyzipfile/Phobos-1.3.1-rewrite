...
public class GuiUtil {
    ...
    public static boolean isHovered(Frame element, int mouseX, int mouseY) {
        return GuiUtil.isHovered(element.getPosX(), element.getPosY(), element.getWidth(), element.getHeight(), (float)mouseX, (float)mouseY);
    }
    ...
    public static double getDistance(SnapPoint point, HudElement element) {
        ...
        switch (orientation) {
            ...
            case VERTICAL_CENTER: {
                return Math.abs(point.getLocation() - (element.getX() + element.getWidth() / 2.0f));
            }
            case HORIZONTAL_CENTER: {
                return Math.abs(point.getLocation() - (element.getY() + element.getHeight() / 2.0f));
            }
        }
        return -1.0;
    }
    ...
    public static void updatePosition(SnapPoint point, HudElement element, boolean inverted) {
        ...
        switch (orientation) {
            ...
            case VERTICAL_CENTER: {
                element.setX(point.getLocation() - element.getWidth() / 2.0f);
                break;
            }
            case HORIZONTAL_CENTER: {
                element.setY(point.getLocation() - element.getHeight() / 2.0f);
            }
        }
    }
