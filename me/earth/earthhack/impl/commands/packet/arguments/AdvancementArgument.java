...
extends AbstractArgument<Advancement> {
    public static final AdvancementManager MANAGER = new AdvancementManager(null);
    ...
    public Advancement fromString(String argument) throws ArgParseException {
        Advancement advancement = this.getAdvancementStartingWith(argument);
        if (advancement == null) {
    ...
    public PossibleInputs getPossibleInputs(String argument) {
        ...
        Advancement advancement = this.getAdvancementStartingWith(argument);
        if (advancement != null) {
            inputs.setCompletion(TextUtil.substring(advancement.getId().toString(), argument.length()));
        }
        return inputs;
    }
    ...
    private Advancement getAdvancementStartingWith(String name) {
        for (Advancement advancement : MANAGER.getAdvancements()) {
            if (!TextUtil.startsWith(advancement.getId().toString(), name)) continue;
            return advancement;
        }
        return null;
    }
