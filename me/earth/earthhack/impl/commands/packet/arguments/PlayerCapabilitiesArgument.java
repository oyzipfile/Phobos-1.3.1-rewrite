...
public class PlayerCapabilitiesArgument
extends AbstractArgument<PlayerCapabilities>
implements Globals {
    ...
    @Override
    public PlayerCapabilities fromString(String argument) throws ArgParseException {
        if (PlayerCapabilitiesArgument.mc.player == null) {
            throw new ArgParseException("Minecraft.Player was null!");
        }
        String[] split = argument.split(",");
        boolean damage = split.length > 0 ? Boolean.parseBoolean(split[0]) : PlayerCapabilitiesArgument.mc.player.capabilities.disableDamage;
        boolean flying = split.length > 1 ? Boolean.parseBoolean(split[1]) : PlayerCapabilitiesArgument.mc.player.capabilities.isFlying;
        boolean allow = split.length > 2 ? Boolean.parseBoolean(split[2]) : PlayerCapabilitiesArgument.mc.player.capabilities.allowFlying;
        boolean creative = split.length > 3 ? Boolean.parseBoolean(split[3]) : PlayerCapabilitiesArgument.mc.player.capabilities.isCreativeMode;
        boolean edit = split.length > 4 ? Boolean.parseBoolean(split[4]) : PlayerCapabilitiesArgument.mc.player.capabilities.allowEdit;
        float flySpeed = split.length > 5 ? (float)ArgParseException.tryDouble(split[5], "speed") : PlayerCapabilitiesArgument.mc.player.capabilities.getFlySpeed();
        float walkSpeed = split.length > 6 ? (float)ArgParseException.tryDouble(split[5], "walk") : PlayerCapabilitiesArgument.mc.player.capabilities.getWalkSpeed();
