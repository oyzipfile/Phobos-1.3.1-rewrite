...
import me.earth.earthhack.impl.commands.packet.factory.playerlistitem.AddPlayerDataArgument;
...
public class SPacketPlayerListItemFactory
extends DefaultFactory {
    ...
    private static final PacketArgument<SPacketPlayerListItem.Action> ACTION_ARGUMENT = new EnumArgument<SPacketPlayerListItem.Action>(SPacketPlayerListItem.Action.class);
    private static final PacketArgument<SPacketPlayerListItem.AddPlayerData[]> ARRAY_ARGUMENT = new FunctionArrayArgument<SPacketPlayerListItem.AddPlayerData>(SPacketPlayerListItem.AddPlayerData[].class, ARGUMENT, SPacketPlayerListItem.AddPlayerData[]::new);
    ...
    @Override
    protected Packet<?> instantiate(Constructor<? extends Packet<?>> ctr, Object[] parameters) throws NoSuchFieldException, IllegalAccessException {
        ...
        for (int i = 1; i < parameters.length; ++i) {
            ...
            if (o.getClass().isArray()) {
                for (int j = 0; j < Array.getLength(o); ++j) {
                    packet.getEntries().add((SPacketPlayerListItem.AddPlayerData)Array.get(o, j));
                }
                continue;
            }
            if (!(o instanceof Iterable)) continue;
            for (Object data : (Iterable)o) {
                packet.getEntries().add((SPacketPlayerListItem.AddPlayerData)data);
            }
        }
        return packet;
    }
    ...
    static {
        try {
            Constructor ctr = SPacketPlayerListItem.class.getDeclaredConstructor(SPacketPlayerListItem.Action.class, Iterable.class);
            ITERABLE_ARGUMENT = new GenericIterableArgument<SPacketPlayerListItem.AddPlayerData>(ctr, 1, ARGUMENT);
        }
