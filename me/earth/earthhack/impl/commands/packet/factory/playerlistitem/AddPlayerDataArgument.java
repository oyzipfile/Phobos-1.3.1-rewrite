...
extends AbstractArgument<SPacketPlayerListItem.AddPlayerData> {
    ...
    private static final PacketArgument<GameType> GAME_TYPE_ARGUMENT = new EnumArgument<GameType>(GameType.class);
    ...
    @Override
    public SPacketPlayerListItem.AddPlayerData fromString(String argument) throws ArgParseException {
        String[] split = argument.split(",");
        if (split.length < 4) {
            throw new ArgParseException("Expected 3+ Arguments for EntityPlayerMP, but found: " + split.length + "!");
        }
        GameProfile profile = GAME_PROFILE_ARGUMENT.fromString(split[0] + "," + split[1]);
        ...
        return new SPacketPlayerListItem.AddPlayerData(sPacketPlayerListItem, profile, latency, gameType, component);
    }
    ...
    public PossibleInputs getPossibleInputs(String argument) {
        ...
        switch (split.length) {
            ...
            case 1: {
                PossibleInputs g = GAME_PROFILE_ARGUMENT.getPossibleInputs(split[0]);
                return inputs.setCompletion(g.getCompletion()).setRest(g.getRest() + ",latency,GameType,TextComponent");
            }
            case 2: {
                PossibleInputs g2 = GAME_PROFILE_ARGUMENT.getPossibleInputs(split[0] + split[1]);
            ...
            case 4: {
                PossibleInputs type = GAME_TYPE_ARGUMENT.getPossibleInputs(split[2]);
