...
final class ListenerDiggingNoEvent
extends ModuleListener<AntiSurround, PacketEvent.NoEvent<CPacketPlayerDigging>> {
    private final AntiSurroundFunction function;

    public ListenerDiggingNoEvent(AntiSurround module) {
        super(module, PacketEvent.NoEvent.class, -1000, CPacketPlayerDigging.class);
        this.function = new PreCrystalFunction(module);
    }

    @Override
    public void invoke(PacketEvent.NoEvent<CPacketPlayerDigging> event) {
        if (event.isCancelled() || !event.hasPost() || ((CPacketPlayerDigging)event.getPacket()).getAction() != CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK || ((AntiSurround)this.module).holdingCheck() || !((AntiSurround)this.module).preCrystal.getValue().booleanValue() || !((ICPacketPlayerDigging)event.getPacket()).isClientSideBreaking()) {
            return;
        }
