...
implements Remapper {
    ...
    public void remap(ClassNode cn, Mapping mapping) {
        for (FieldNode fn : cn.fields) {
            ...
            if (!this.hasShadowAnnotation(fn)) continue;
    ...
    private boolean hasShadowAnnotation(FieldNode fn) {
        if (fn.visibleAnnotations != null) {
            for (AnnotationNode an : fn.visibleAnnotations) {
                if (an == null || !"Lorg/spongepowered/asm/mixin/Shadow;".equals(an.desc)) continue;
                return true;
            }
        }
        if (fn.invisibleAnnotations != null) {
            for (AnnotationNode an : fn.invisibleAnnotations) {
                if (an == null || !"Lme/earth/earthhack/installer/srg2notch/RemapFieldName;".equals(an.desc)) continue;
                return true;
            }
        }
        return false;
